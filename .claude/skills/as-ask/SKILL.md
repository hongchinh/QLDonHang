...
Read project documentation when relevant: `CLAUDE.md`, `README.md`, `docs/SUMMARY.md`, and any topic file directly tied to the task. Only gather what is needed for the current task. Skip if no docs exist.
...
Use the `AskUserQuestion` tool to collect answers from the user.

Rules:

- Prefer **multiple-choice options** (2–4 choices) when practical
- Use free-form questions only when no reasonable options exist
- Do not ask questions already answered by project documentation
- Do not ask about implementation details prematurely
- Keep each question focused on a single decision; bundle related decisions only when they are tightly coupled
...
3. Recommend the appropriate next skill based on complexity:
   - If no code work is needed, just answer the question directly.
