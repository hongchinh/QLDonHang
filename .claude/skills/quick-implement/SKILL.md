...
3. Use `AskUserQuestion` to ask the user if they want to initiate a handoff to the `write-plan` skill.
...
2. Read project context when relevant: `CLAUDE.md`, `docs/SUMMARY.md`, plus the minimum necessary code paths.
3. Confirm the task still passes the Scope Gate.
4. If ambiguity remains, ask clarifying questions via `AskUserQuestion` before coding.
...
4. Update documentation if minor behavior or domain rules changed (e.g., small updates to `docs/project-pdr/` or component-specific READMEs). Do not touch architecture docs; if architecture changed, this task should have been escalated.
...
- Use `as-fix` when the task is primarily debugging an issue.
