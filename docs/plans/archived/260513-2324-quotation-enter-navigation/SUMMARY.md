...
- [x] **Phase 1 [S]: Stable Grid Focus API** - Add stable product-code cell IDs and expose a method to focus/create the first row.
- [x] **Phase 2 [M]: General Info Keyboard Handler** - Add scoped `Enter` / `Shift+Enter` / `Ctrl+S` handling in the quotation form.
- [x] **Phase 3 [S]: Verification and Polish** - Run typecheck/tests, manually validate keyboard flows, and update the keyboard shortcut guide in the grid footer to include `Enter` / `Shift+Enter` / `Ctrl+S`.
