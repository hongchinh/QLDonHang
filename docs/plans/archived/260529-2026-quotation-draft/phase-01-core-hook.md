...
Tạo `frontend/src/features/quotations/use-quotation-draft.ts` chứa:
- Pure functions: `readQuotationDraft`, `writeQuotationDraft`, `deleteQuotationDraft`
- Hook: `useQuotationDraft` quản lý `hasDraft`, `draftSavedAt`, debounced write, `clearDraft`

Không chạm vào `quotation-form-page.tsx` ở phase này.

## Files

- `frontend/src/features/quotations/use-quotation-draft.ts` (tạo mới)
- `frontend/src/features/quotations/use-quotation-draft.test.ts` (tạo mới)

## Tasks

### Task 1.1 — Viết toàn bộ test file trước khi implement

1. Tạo `frontend/src/features/quotations/use-quotation-draft.test.ts`:
...
2. Chạy test để xác nhận FAIL:
   ```bash
   cd frontend && npx vitest run src/features/quotations/use-quotation-draft.test.ts
   ```
   Expected: FAIL — `Cannot find module './use-quotation-draft'`

### Task 1.2 — Implement `use-quotation-draft.ts`

3. Tạo `frontend/src/features/quotations/use-quotation-draft.ts`:
...
6. Commit:
   ```bash
   git add frontend/src/features/quotations/use-quotation-draft.ts frontend/src/features/quotations/use-quotation-draft.test.ts
   git commit -m "feat: add useQuotationDraft hook with localStorage persistence"
   ```
...
- [ ] `use-quotation-draft.ts` tồn tại, export đầy đủ: `readQuotationDraft`, `writeQuotationDraft`, `deleteQuotationDraft`, `useQuotationDraft`, `QuotationDraftCustomer`, `QuotationDraftStorage`, `UseQuotationDraftOptions`, `UseQuotationDraftResult`
